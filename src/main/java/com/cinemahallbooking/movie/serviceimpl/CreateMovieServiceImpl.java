...
public class CreateMovieServiceImpl implements CreateMovieService {
	...
	public ResponseEntity<?> save(CreateMovieModel request) {
	    ...
	    if (existingMovie.isPresent()) {
	        return new ResponseEntity<>("Movie with title '" + request.getMovieTitle() + "' already exists!", HttpStatus.CONFLICT);
	    }

	    // Fetch locations based on the provided location IDs
	    List<String> locationIds = request.getLocations().stream().map(LocationModel::getId).collect(Collectors.toList());
	...
	public List<CreateMovieModel> getAllCreatedMovies() {
	    ...
	    movies.forEach(movie -> {
	        List<String> cinemaHallIds = movie.getCinemaHalls().stream().map(CinemaHallModel::getId).collect(Collectors.toList());
...
//	}
	public ResponseEntity<?> updateMovie(String movieId, CreateMovieModel updatedMovie) {
	    ...
	    List<CinemaHallModel> filteredCinemaHalls = allCinemaHalls.stream()
	        .filter(cinemaHall -> locations.stream()
	            .map(LocationModel::getLocationName)
	    ...
	    for (Map.Entry<String, Object> entry : updates.entrySet()) {
	        ...
	        switch (key) {
	            ...
	            case "releaseDate":
	                ...
	                break;
	            case "locations":
	                List<String> locationIds = ((List<Map<String, String>>) value)
	                        .stream().map(loc -> loc.get("id")).collect(Collectors.toList());
	                ...
	                updatedCinemaHalls.addAll(allCinemaHalls.stream()
	                    .filter(hall -> updatedLocations.stream()
	                        .map(LocationModel::getLocationName)
	            ...
	            case "cinemaHalls":
	                ...
	                for (Map<String, Object> hallUpdate : halls) {
	                    ...
	                    CinemaHallModel updatedHall = updatedCinemaHalls.stream()
	                            .filter(hall -> hall.getId().equals(hallId))
	                            .findFirst()
	                            .orElseGet(() -> {
	                    ...
	                    if (hallUpdate.containsKey("showTimings")) {
	                        List<Map<String, Object>> showTimings = (List<Map<String, Object>>) hallUpdate.get("showTimings");
	                        ...
	                        for (Map<String, Object> showUpdate : showTimings) {
	                            ...
	                            ShowTimingModel updatedShow = updatedHall.getShowTimings().stream()
	                                    .filter(existingShow -> existingShow.getTime().equals(time))
	                                    .findFirst()
	                                    .orElseGet(() -> {
	                            ...
	                            if (showUpdate.containsKey("availableSeats")) {
	                                List<Map<String, Object>> seatData = (List<Map<String, Object>>) showUpdate.get("availableSeats");

	                                List<SeatModel> updatedSeats = seatData.stream()
	                                        .map(seat -> {
